...
**A uniform modern scan is nearly constant; a scanned book is not.** For 23 of the 25 paged series here the min and max sit within 0.02 of the median, and only Dragon Ball Super (1.35–1.56) and Death Note (1.50–1.58) carry a tail at all. That constancy is a property of these files and not of comics: on Kavita's demo [below](#a-second-library-one-anybody-can-re-run-kavitas-public-demo), four of five paged series vary by 0.05 or more — Captain Marvel Adventures spans 1.27–1.47 — because Golden Age comics are scanned from mixed sources and trim sizes. Nothing there threatens the threshold (the demo's tallest page is 1.52, and a median sits in the middle of a spread rather than at its top), but a book scanned from mixed sources is the case where a median could drift up towards 1.8, and it is the false positive to watch for.
