...
**And then in the embedded engine itself**, which is the measurement that decides rather than suggests: Chrome for Android is a different APK from the WebView an app embeds, and Chromium's hyphenation dictionaries are a downloadable component. Run inside a real `webview_flutter` on Android 16 (`…; wv) … Chrome/151.0.7922.199`, a build apart from the Chrome 153 on the desktop beside it): **hyphenation on, `lang` heard, 5 of 5 separated.** A third fact came free and is worth more than it cost — an **unknown language tag hyphenates nothing at all**. The engine does not fall back to English; it declines. So "a book whose language is not known is set ragged right" needs nothing built, only nothing undone.

Two limits on that run. The probe's **absolute break positions are bracketed rather than measured**: its two independent methods disagree by exactly two characters on all ten pairs, with the TeX break exactly between them, and neither offset has been explained — so read a position as a midpoint, and read the `fr` ≠ `en` comparison as sound, a constant offset cancelling between two columns taken with one instrument. And **iOS was not measured this way at all**: there is no Mac here, so Safari stands in for WKWebView — a far stronger proxy than Chrome was for the System WebView, both being WebKit on a platform that permits no other engine, but a proxy.
