...
**What a hero pictures is the chapter its button opens, not the series it belongs to.** Both heroes — the series screen's and Home's Continue card — swap their cover for the resume target's whenever `entryPictured` (`resume_point.dart`) says the series is under way: started, and not everything read. That includes a next volume nobody has opened yet: finishing a volume leaves the next one untouched, and a hero that pictured only a chapter with `pagesRead > 0` fell back to the *series* cover at exactly the moment a reader turned a volume's last page, saying nothing about which volume came next. The **page behind** a hero follows the stricter `entryUnderWay` — the same fields plus the chapter's own `pagesRead > 0` — because the first page of something unread is a spoiler where its cover is only what the row below already shows. Both predicates live in the file that decides where reading resumes, so neither screen can pick a different chapter from the other. The backdrops still differ, and deliberately — Home draws the resume point's chapter as soon as there is one, page 0 included, where the series screen refuses an unopened page. The series cover stands in two cases that are different facts rather than one: on Home the chapter is merely **not known yet** (the card is drawn as soon as its series is, and fills the chapter in behind), while on either screen a button that *starts* a series or offers it *again* has no chapter you are inside. A **volume with no chapter breakdown is drawn by its volume cover** (`entryCoverUrl`), the choice the rows below already make: the volume is the reading unit there, and asking for the placeholder chapter's cover instead would fetch and store a second copy of the picture on the row the hero opens, since the shared image cache keys on the URL. The **ring follows the picture** — a bar under a cover means "how far through the thing pictured", the rule every chapter row and library tile obeys — so it is the chapter's progress while the cover is the chapter's, and series-wide only when the cover is. Covered by `test/home_hero_test.dart` and `test/series_hero_test.dart`.
