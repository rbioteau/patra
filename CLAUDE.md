...
The Android **adaptive** icon is deliberately *not* generator-managed: its foreground is `drawable/verso_mark.xml`, the same vector the splash screen uses, so the launcher icon and the splash cannot drift apart and neither can pixelate. `flutter_launcher_icons` only rasterises PNG foregrounds and would rewrite `mipmap-anydpi-v26/ic_launcher.xml` with a 16% inset that double-pads a vector already carrying its own padding — so it keeps no `adaptive_icon_*` keys, and running it leaves the adaptive icon and `values/colors.xml` (`verso_ink` = `#16141C`) untouched. Only 66dp of the 108dp canvas is guaranteed visible, which is why the mark sits at ~48% of the viewport width in that vector; change it there and both the icon and the splash follow.
