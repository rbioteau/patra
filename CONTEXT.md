...
**Reflowable content**:
What a [[Chapter]] is made of when it is words rather than pictures: **Kavita** lays the text out and hands the app one page at a time, so a page is a slice the server chose and not a picture with a size — change the text size or the screen and the same words break somewhere else. **The app rewrites that page and hands it to the platform's own engine** (ADR-0013): what Kavita scoped is stripped of everything that could execute or reach the network, and its pictures pointed at files already on the device, before any of it is rendered — so a book is composed as its publisher set it, while the size, the leading and the [[Reading face]] stay the reader's. Its [[Reading progress]] is therefore a page the server counted and may count again differently, and everything built on measuring pages — the [[Strip]], the [[Spread]], [[Magnifying]], and the measured half of the [[Detected direction]] — has nothing to measure and is never offered for it. What a book does have is the [[Declared direction]], which is read rather than measured: the book's own stylesheet arrives with every page.
...
**Reading direction**:
Which way the reader advances through a chapter: left-to-right, right-to-left, or vertical scrolling. One setting with three values — the third is a direction like the other two, not a separate mode sitting beside them. Which one a chapter opens in is answered by a chain, each rung only asked when the one above it has no answer: the [[Series direction]], then the [[Library direction]], then the [[Detected direction]] — a guess never beats a choice — and last the left-to-right a chapter has always opened in. No rung in it belongs to a [[Profile]] or to a device: a direction is a property of the work, and a default held for every series at once is the wrong shape for one. It is asked of [[Reflowable content]] too, and the one answer sets both halves at once — which way the words run, and which way the pages turn — since a book that reads from the right while its pages turn the other way is worse than one that does neither. But **nothing is measured of a book**, so the only evidence its guess has is the [[Declared direction]]: a book declaring nothing opens left to right, and the shelf it happens to sit on cannot turn it. Only whether it reads from the right is asked there either: [[Vertical scrolling]] names a strip nobody scrolls through in a book, whose pages are the server's and are turned.
...
**Detected direction**:
The [[Reading direction]] the app infers for a work it has been told nothing about, since Kavita exposes no direction of its own. Three kinds of evidence, and they answer different questions rather than competing. The [[Declared direction]] is asked first, and is the only one of the three that is a statement rather than an inference. Then the two inferences, and **both are about scans**: whether a work is vertical is the pages' (a median tallness of 1.8 and above, measured on the chapter being read, which is the only place page dimensions reach the app), and which way it goes when it is not is the library type's, a manga library reading right to left and every other one the way a chapter always has — **that type being the one the device's own catalogue holds, never the one the chapter states**, since `chapter-info` declares a library type that no Kavita has ever populated and so calls everything manga. A shelf the device has not learned yet is no witness at all, and the guess then says nothing rather than falling back to the one type that carries a direction. Neither inference is asked of [[Reflowable content]], because neither has anything to go on there — a book carries no page dimensions at all, and a convention about how *scans* are bound says nothing about words. Genres and tags carry no usable signal and are not consulted. It is a guess and never a choice: anything anybody actually chose outranks it, and the reader says where a direction came from rather than presenting a guess as one.
_Avoid_: automatic direction, smart default, heuristic (in prose)

**Declared direction**:
The direction a book states it is written in, read out of the book's own CSS: a `direction: rtl` in the stylesheet **Kavita** inlines into every page of [[Reflowable content]]. It is the only witness left, and that is the server's doing rather than a choice of ours — `PrepareFinalHtml` copies the classes off a book's `<html>` and `<body>` and returns their inner HTML in a wrapper of its own, so both conventional places for a base direction, a `dir` attribute and an `<html lang>`, are thrown away while the stylesheet survives. **A declaration is not a match**: the server's rescoping leaves declarations behind that apply to nothing, so what counts is a rule whose selector still picks out the page as a whole, and a rule that only applies under an `@media` counts as nothing. Only right-to-left is read, because `direction: ltr` is the CSS default and cannot be told from a stylesheet that says nothing. It is evidence for the [[Detected direction]] and never a direction of its own, so anything anybody chose still outranks it — and it is read per [[Chapter]] and recorded per [[Series]], a direction read off one chapter of a work being a direction for the work.
_Avoid_: book direction (that is the answer, not this evidence for it), dir, RTL support
...
**Reading face**:
The typeface a book's pages are set in. The choice is a **kind of type** rather than a font — the book's own, the app's serif, the app's sans — and the family behind one of the three is the book's business rather than the app's. The **book's own** is the default, and means the face the book's own stylesheet asks for, read out of the page Kavita hands over; a book that asks for nothing, or whose font cannot be had, is set in the app's sans, which is what every book was set in before there was anything to choose. A [[Preference]]: it belongs to the [[Profile]] and follows their eyes, like the text size and the line spacing, and the server is never told — though the server's own client answers the same question the same way, by leaving the book's stylesheet to win. It governs the whole page — prose, headings, quotations and list items — but **not code** (`pre`, `code`, `kbd`, `samp`, `tt` and what they hold), whose columns are aligned by a fixed width a face for prose would take away, and which keeps the book's own face or the engine's monospace; and not the page counter, which is the app's own furniture and stays in the serif whatever is chosen. It is the one deliberate hole in the serif rule: that rule keeps the wordmark and the titles of works distinct from everything else, and prose set in a serif puts neither at risk.
_Avoid_: font (that is one file of a family, and each of ours ships as a single variable file)

**Hyphenation**:
Breaking a word across two lines where the language allows it, with the mark drawn at the break. It is the engine's work rather than the app's arithmetic, and the dictionary it hyphenates from is named by the **book's own declared language** — which is why a book whose language is not known is set ragged right instead: justifying a narrow column with nothing to break the long words with opens rivers of white, and a break with no mark drawn reads as a fault rather than as typography. Not a [[Preference]]: nobody is asked, because there is no version of it a reader would choose against.
_Avoid_: word wrap (that is where a line ends, not where a word may be cut), soft hyphen (that is one publisher's own mark placed inside the text, and only one of the places a break may fall), justification (that is what hyphenation is *for*, not what it is)
